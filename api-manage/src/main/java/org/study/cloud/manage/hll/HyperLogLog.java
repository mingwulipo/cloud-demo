package org.study.cloud.manage.hll;
...
public class HyperLogLog {
    ...
    private static int log2m(double rsd) {
        double a = (1.106 / rsd) * (1.106 / rsd);
        double v = Math.log(a) / Math.log(2);
        return (int) v;
    }
    ...
    public boolean offerHashed(int hashedValue) {
        String binaryString = Integer.toBinaryString(hashedValue);
    ...
    public long cardinality() {
        ...
        if (estimate <= (5.0 / 2.0) * count) {  //小数据量修正
            double a = linearCounting(count, zeros);
            return Math.round(a);
        } else {
